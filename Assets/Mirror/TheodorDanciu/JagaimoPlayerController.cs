...
public class JagaimoPlayerController : NetworkBehaviour
{
    //Variables
    #region Variables
    [SerializeField] 
    ...
    private JagaimoPlayerCamera      pC;
    private JagaimoPlayerInteraction pI;
    ...
    private JagaimoPlayerAnimation pA;
    [SerializeField]
    ...
    private static JagaimoPlayerControls controls;
    public static JagaimoPlayerControls Controls
    {
        get
        {
            if (controls != null) { return controls; }
            return controls = new JagaimoPlayerControls();
        }
    }
    ...
    {
        pA = GetComponent<JagaimoPlayerAnimation>();
        pI = GetComponent<JagaimoPlayerInteraction>();
        sM = FindObjectOfType<SoundManager>();
        rebindingDisplay = FindObjectOfType<RebindingDisplay>();
        speed = walkSpeed;
        Physics.queriesHitBackfaces = true;
        pC = GetComponent<JagaimoPlayerCamera>();
   ...
   private void FixedUpdate()
   {
       ...
       if (playerMovement != pM.INTERACTING && playerMovement != pM.STANDING &&
           pC.cameraState != JagaimoPlayerCamera.cS.FREECAMUNCONSTRAINED)
       {
           ...
           {
               particles.CreateParticle(ScriptableParticles.Particle.LandImpact,
           ...
           else if (!grounded && !climbing)
           {
               ...
               if (jumpValue > 0.1f && pI.WallKick())
               {
                   ...
                   particles.CreateParticle(ScriptableParticles.Particle.AirImpact,
               ...
               else
               {
                   if (touchingWall)
                   {
                       rb.AddForce(-rotationPosition.TransformDirection(movementValue) * (Time.deltaTime * 100),
                           ForceMode.Impulse);
           ...
           {
               particles.CreateParticle(ScriptableParticles.Particle.JumpDust,
                   new Vector3(transform.position.x, transform.position.y - 1, transform.position.z));
            ...
            case pM.WALKING:
                if (crouchValue > 0.1f && crouching == false && sliding == false)
                {
                    ...
                    if (grounded)
                    {
                        ...
                        {
                            sliding = true;
                            particles.CreateParticle(ScriptableParticles.Particle.Fuse,
    ...
    public void Jump(InputAction.CallbackContext ctx)
    {
        ...
        if (pC.cameraState == JagaimoPlayerCamera.cS.FREECAMUNCONSTRAINED)
        {
            pC.MoveFreeCamY(true, ctx.ReadValue<float>());
        }
        else if (pC.cameraState != JagaimoPlayerCamera.cS.FREECAMCONSTRAINED)
        {
    ...
    public void Crouch(InputAction.CallbackContext ctx)
    {
        if (!grounded)
        {
            pA.CheckToChangeState(crouchValue >= 0.1f ? "CrouchingIdle" : "Idle");
        }
